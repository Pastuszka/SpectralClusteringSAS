%macro spccSpectralize1;
	...
	proc iml;
		...
		if &l_correct & &gauss_correct & &knn_correct & correct then call symput('return_code', '0');
...
%macro spccSpectralize2;
	...
	proc iml;
		...
		m = {0 1, 0 2, 0 3, 0 4};
		expected = {1 1 1, -2.1638 0.2889 1, 2.1639 -0.2888 1, -1 -1 1};
		out = spccSpectralize(m, 2, 'normalizedSym', 'gaussian', 0.5, 'mutual_knn', 2);
		correct = all(out = expected);
		if &l_correct & &gauss_correct & &knn_correct & correct then call symput('return_code', '0');
		else call symput('return_code', '1');
	quit;
